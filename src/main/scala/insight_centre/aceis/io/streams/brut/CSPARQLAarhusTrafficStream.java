	...
	protected SensorObservation createObservation(Object objData) {
		...
		try {
			...
			data = new AarhusTrafficObservation(Double.parseDouble(streamData.get("REPORT_ID")),
					...
					!streamData.get("Point2_Long").isEmpty() ? Double.parseDouble(streamData.get("Point2_Long")) : 0.0, null, null, streamData.get("TIMESTAMP"), getIRI());
			...
			Double distance = Double.parseDouble(distanceByReportId.get(streamData.get("REPORT_ID")) + "");
